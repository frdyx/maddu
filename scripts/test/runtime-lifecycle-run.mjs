...
async function main() {
  ...
  // ── 5. attempts, record, terminals, FileStore ──
  {
    ...
    ok('record() writes app.* and observation events with operation/causes/subjectDigest through the store', (await a.record('ACTION_PROPOSED', { kind: 'send' }, { operation: 'op-1', causes: [r.id], subjectDigest: e.subjectDigest })).seq === 7 && (await rejects(a.record('ACTION_PROPOSED', {}, { operation: 'op-1' })))?.code === 'duplicate_operation');
