...
// being indistinguishable: planDigest / planTaskCount / tasksTruncated (+ a
// bounded `tasks` diagnostics array) for kind:'project-test', and
...
export const EVENT_SCHEMA = {
  ...
  VERIFICATION_RAN: { summary: "A verification RECEIPT appended from the runner's in-process result (never a re-read state file) and referencing its paired VERIFICATION_STARTED — the recency/success readouts trust this tamper-detecting spine receipt, not a hand-writable projection.", data: { conditionCount: 'number?', conditionPlanDigest: 'string?', planDigest: 'string?', planTaskCount: 'number?', tasks: 'array?', tasksTruncated: 'boolean?', allMet: 'boolean?', complete: 'boolean', conditions: 'array?', counts: 'object|null', kind: 'string', metCount: 'number?', objective: 'string|null?', pendingCount: 'number?', profile: 'string|null', result: 'string', setAt: 'string|null?', startedId: 'string', verifiable: 'number?' } },
