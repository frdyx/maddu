...
Current version: **v1.4.0** ([changelog](../CHANGELOG.md)) — Empirical insights, a plugin loader, and a tighter core. The first data-driven release: a cross-project usage audit found 90 of 142 event types never fired in any real run, and acts on it. New read-only `maddu insights` (`/maddu-insights`) reads `.maddu` spines across registered workspaces and classifies every event type load-bearing / occasional / single-project / dormant / dead. New `maddu plugin` (`/maddu-plugin`) loads capabilities that live outside the core from a `plugin.json` manifest (files-only enable-state; user-added gated by `--trust`); **comms (Telegram / Discord / Email) is now the first plugin — off by default**, auto-migrated on `upgrade` for repos that had it enabled. Dead subsystems wired into the default flow: `slice-stop` auto-detects skill candidates (rule-#9 gauntlet) and the coordinator enforces a per-phase review. 55 verbs, all classified agent vs operator (`surface` field). The 8+1 hard rules are unchanged; `maddu audit` reports 6/6. Carries forward v1.3.0's charter + default pipelines (see [`charter.md`](charter.md)). License: Apache-2.0. Repo: <https://github.com/frdyx/maddu>.
