...
| Planning & autonomy | `pipeline`, `plan`, `goal`, `phase`, `loop`, `coordinator`, `team`, `advise`, `suggest`, `handoff` | First-class auto-revising planning: pipelines and goals decompose work (`goal` carries measurable success conditions), `phase` and `loop` are the iteration primitives, the `coordinator` fans work across a `team`, `advise`/`suggest` route intent, and `handoff` curates the cross-session "▶ RESUME HERE" briefing. |
| Quality & review | `review`, `test`, `self-test`, `lint`, `format`, `doctor`, `architecture` | Keep the surface honest — gate slices through host-project `test` (legacy runner or adaptive profiles), framework-source `self-test`, `lint`, `format`, `doctor`'s 8+1 hard rules, and `architecture`'s drift check (the declared architecture contract vs the real import graph — forbidden edges, cycles, undeclared areas — with a `failOn` baseline ratchet). |
