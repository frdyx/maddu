...
| Quality & review | `review`, `test`, `self-test`, `lint`, `format`, `doctor` | Keep the surface honest — gate slices through host-project `test` (legacy runner or adaptive profiles), framework-source `self-test`, `lint`, `format`, and `doctor`'s 8+1 hard rules. |
