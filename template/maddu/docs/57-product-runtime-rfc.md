...
| Gates | Built-in and operator gate modules are discovered from disk; contexts carry repository/state roots; a gate-receipt append can fail while the gate result still returns. | Add a runtime gate registry and an explicit evidence-durability result. The existing runner is not a fail-closed action boundary. |
