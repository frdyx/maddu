...
| P4 | landed (v1.149.0) | `runtime/execution/{decision,boundary}.mjs` | Per-boundary frozen policy (enforced / requireApproval / ttl); `decide` reads gate coverage from the run's evidence, binds the action exactly, escalates through `APPROVAL_REQUESTED`/`APPROVAL_DECIDED` (human producer, single use, expiring) and issues an HMAC-signed single-use handle on allow; shadow records `wouldDecide`. `execute`/`release` re-bind the handle to the parameters, resource version or artifact presented now, let host `authorize()` deny, consume through the store's one-operation-key rule, record `unknown` for a throwing side effect, and `reconcile` resolves the crash window from host state. The synthetic pilot, receipts export and streaming are P5 |
| P5 | landed (v1.150.0) | `runtime/execution/observe.mjs`, `runtime/core/{minimize,receipt,measure}.mjs`, `scripts/test/__fixtures__/runtime-pilot-host.mjs`, `scripts/test/runtime-pilot-synthetic.mjs` | Context references and host-observed model calls store references and digests only (a body key or a secret shape in metadata is refused; timeout is `unknown`); shape-based `minimize` with stated limits; portable receipt (`exportReceipt`/`verifyReceipt`) with known omissions, fail-closed on secret shapes, always `authority: unsigned`; `measureRun` counts would-block vs blocked and unknown/unresolved without rates or a clock. The synthetic pilot runs the public surface end to end under a FileStore with a scripted model, an in-memory CRM crash window and an outbox that refuses streaming (V11/V13/V14/V15); everything simulated is labelled simulated. Signed receipts, producer keys and an external witness remain `not_supplied` |
| P8 | landed (v1.151.0) | `runtime/index.d.ts`, `package.json` (`exports["./runtime"].types`), `docs/58-embedded-runtime.md`, README "Two ways in", `scripts/test/runtime-external-consumer.mjs` | Public typed entry point under the existing `maddu` package (ADR-002; no separate name claimed); the adoption page states supported modes, tested guarantees with limits, the compatibility policy (contract strings are the surface), the threat-model delta with residuals and the pilot's baseline counts; the external-consumer fixture packs the checkout, installs the tarball offline into a fresh consumer, runs a mini pilot through `maddu/runtime` only (V25, V01), checks the d.ts against every export, reinstalls in place, and audits README + docs/58 for claims beyond the tested scope (V27) |
| P6–P7 | not started (optional economy experiment; needs the operator's go/no-go on the P5 evidence) | — | — |
