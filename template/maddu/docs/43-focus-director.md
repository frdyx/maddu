...
| Stage | Mechanism |
|---|---|
| **Per-turn tag** | On every `session heartbeat` (and each `slice-stop`, as a floor), a **deterministic** tagger scores the turn `toward` / `lateral` / `away` of the goal — token overlap between your current focus text and the goal's objective + success conditions. No LLM; zero cost. Appended as `FOCUS_TAGGED`. |
