...
**Verifiable, not just declared.** The spine's status as source of truth is a promise the framework makes — and one it must let the operator check. `maddu spine verify` walks every segment under `.maddu/events/` and confirms: NDJSON parseability, event-id uniqueness, segment continuity, timestamp monotonicity within each segment, and referential integrity across types (every `APPROVAL_DECIDED` resolves to a real `APPROVAL_REQUESTED`; every `LANE_RELEASED` follows a matching `LANE_CLAIMED`; every `SESSION_CLOSED` references a registered session; etc.). `maddu doctor` calls into the verifier on every run (up to a 50k-event cap, above which the operator runs `verify` manually) so a green doctor report is a real statement about the framework's bedrock. The verifier is strictly read-only — if it flags an issue, the operator decides how to address it (manual edit + slice-stop, checkpoint rollback, etc.). No auto-repair, ever.
