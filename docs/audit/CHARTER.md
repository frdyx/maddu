...
Reconstructed from primary sources (README, hard-rules, concepts, architecture)
during the 2026-05-24 audit. This is the single reference for "what Máddu is,
what it is meant to be, and the one way it is run." When any doc, agent brief, or
feature disagrees with this file, this file is the intent; the disagreement is
drift to be fixed.
...
*(README.md:7-9; package.json description; docs/00-index.md:5)*
...
## The invariants (the real, stable charter — do not drift)

The **8+1 hard rules** (`docs/hard-rules.md`), enforced by `maddu doctor` on
every install and upgrade. *A repo that violates any of them is not a Máddu repo.*
...
**Rule of thumb the agent brief must state:** *prefer a pipeline.* For any
