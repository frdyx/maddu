...
| Memory & accounting | `memory`, `learn`, `cost`, `usage`, `log`, `import` | Make work outlive the agent — `memory` and hindsight skills persist learning, `learn` mines past sessions for failed→succeeded tool-call pairs and distils durable project corrections (into the project brief + queryable memory), `cost`/`usage` and the receipt `log` account for spend, and `import` retroactively populates the ledger. |
