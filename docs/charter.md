...
| Discovery & observability | `status`, `events`, `spine`, `search`, `brief`, `help`, `audit`, `insights` | Watch and replay the event log — `status` and `events` surface live state, `spine` verifies the append-only truth (never auto-repairs), `search` and `brief` query it, `help` is the on-ramp, `audit` runs the charter-drift check, and `insights` reports empirically what is actually utilized across registered workspaces vs merely defined. |
