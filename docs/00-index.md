...
Current version: **v1.7.0** ([changelog](../CHANGELOG.md)) — Invocation logic. Gives still-dead domains a defined, safe trigger condition wired into the flow instead of forcing them: **trust audit re-runs at slice-stop when the dependency surface changed** (freshness/pin drift on new deps caught in-flow), and a real **coordinator run snapshots HEAD as a checkpoint** (`maddu/checkpoint/*` git tag) before workers mutate the repo. Both cross the rule-#9 gauntlet (allowlist + `triggered_by` + cooldown); opt out by removing a line from `.maddu/config/triggers.json`. **mcp** stays a *directive* (its WHEN can't be detected safely). `maddu insights dead` now separates genuine gaps from **dormant-by-design** capabilities (72 → 54 dead, 18 dormant). v1.6.0 (just prior) brought the **orchestration handoff** (goal success-conditions + `maddu orient` + curated cross-session handoff + cockpit Goal panel); v1.5.0 made Máddu **spawn + track real sub-workers**. 57 verbs, all classified agent vs operator (`surface` field). The 8+1 hard rules are unchanged; `maddu audit` reports 6/6. Carries forward v1.4.0 insights + plugin loader and v1.3.0's charter + default pipelines (see [`charter.md`](charter.md)). License: Apache-2.0. Repo: <https://github.com/frdyx/maddu>.
