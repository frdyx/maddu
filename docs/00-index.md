...
| 34 | [34-threat-model.md](34-threat-model.md) | The operator's security manual — what Máddu enforces (15 attack scenarios + concrete gates) and what it does not — supply chain, spine tampering, verdict machinery, assurance faking, silent non-recording, cost fabrication. *(v1.118.0)* |
...
| 56 | [56-acceptance-proof.md](56-acceptance-proof.md) | Acceptance proofs — a declared command observed to **exit nonzero then zero** while the declared **oracle** bytes stayed frozen and the declared **implementation** bytes moved. Declared on the goal (`goal set --oracle/--impl`), observed by `maddu orient` and `maddu loop ralph` (`--from-goal` or `--verify` + both sets; typed exit-2/3 refusals), derived — never stored, no re-baseline verb — and surfaced by the advisory `acceptance-proven` gate. `live` is the only positive state; `acceptance.maxProofAge` is fail-closed; team-sync is refused, not degraded. Honest scope: **process-level, never "a test failed"** — a weak oracle proven RED→GREEN is still a weak oracle. |
| 57 | [57-product-runtime-rfc.md](57-product-runtime-rfc.md) | **The RFC behind the embedded runtime — runtime track implemented (P0–P5, P8, v1.140.0–v1.151.0); the optional economy (P6–P7) is not started.** §15.1 carries the per-package status; the adoption surface is [58](58-embedded-runtime.md). The RFC and decision record for an opt-in **embedded runtime** for AI-powered products (host-owned model calls; typed evidence; fail-closed gate results; single-use bound decision handles at the host's real action/release boundary; offline receipt verifier with six separate assurance dimensions) and an **optional shadow agent economy** (contribution credits, reliability evidence, integrity incidents, and authority kept as four separate records; frozen task contracts; append-only settlement and corrections; no stakes, no permission changes). Ten ADRs (charter fit, packaging, persistence, event namespace, enforcement posture, economy defaults, identity, privacy, authority, reuse policy), the threat-model delta, work packages P0–P8, and the V01–V27 verification matrix. Pinned to baseline `227c69d` (v1.139.0). *(RFC, 2026-09-20)* |
| 58 | [58-embedded-runtime.md](58-embedded-runtime.md) | **Embed Máddu in an AI-powered product** — the adoption surface for the opt-in `maddu/runtime` subpath export (RFC P8): install and the only supported specifier, the lifecycle in one page from the synthetic pilot, **supported modes** (in-memory store; **experimental** file store; shadow vs enforced boundaries; approval binding; single-use signed handles; **unsigned** receipts; producer keys, witness, signatures and the economy **not supplied**), the tested guarantees V01–V16/V25/V27 each with its stated limit, the compatibility policy (contract strings are the surface; additive within v1), the threat-model delta with residuals, the pilot's baseline counts (no latency, accuracy or savings claim), and the claims statement the `runtime-external-consumer` suite audits. *(v1.151.0)* |
