...
Current version: **v1.6.0** ([changelog](../CHANGELOG.md)) — Orchestration handoff. A goal-anchored session-start briefing for big multi-session projects (inspired by posto's `/orch:status`), built by extending existing primitives. `maddu goal set` now carries measurable `--success "<verify-cmd>::<text>"` conditions; **`maddu orient`** (`/maddu-orient`) is the "session always starts here" briefing — it runs each verify command (✓ met / ○ pending / ? unverifiable) and renders the goal, the curated handoff, and the recent trail; **`maddu handoff set/show`** (`/maddu-handoff`) curates the cross-session "▶ RESUME HERE" narrative; a cockpit Goal panel surfaces it all. v1.5.0 (merged just prior) made Máddu **spawn + track real sub-workers** (coordinator/`team spawn` + OAuth session-tree tracking). 57 verbs, all classified agent vs operator (`surface` field). The 8+1 hard rules are unchanged; `maddu audit` reports 6/6. Carries forward v1.4.0 insights + plugin loader and v1.3.0's charter + default pipelines (see [`charter.md`](charter.md)). License: Apache-2.0. Repo: <https://github.com/frdyx/maddu>.
