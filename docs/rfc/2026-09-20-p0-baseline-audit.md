...
| `node scripts/test/legacy-evidence-vectors.mjs` (new, P1) | 54/54 |
| `node scripts/test/runtime-core-import-boundary.mjs` (new, P1) | 43/43 |
...
undocumented at the anchors. Labels marked ⧗ await the refuter verdicts.
...
| A2-001 | high | cross-partition order is the self-reported `ts` k-way merge; merged-order issues capped at WARN (`spine-append-core.mjs:1335-1354`, `verify.mjs:140-160`) | design-decision ⧗ | Documented in `verify.mjs` ("a timestamp merge across independent replicas is not a causal order"). Runtime requirement: per-run sequence and explicit causal refs (RFC §7.1). Default flat mode unaffected. |
| A2-002 | high | suffix truncation, tail-only edit, and a well-linked forged tail verify clean (`spine-tamper-guard.mjs` tests 11-13 pin them as conceded residuals) | design-decision ⧗ | Documented in `docs/34`, `docs/54`, `hard-rules.md` rule 2. Runtime requirement: retained checkpoint / external witness (RFC §7.4); P1 pins the limit. |
| A2-003 | high | `actor` is a self-declared string; verify checks registration existence, not authenticity (`event-schema.mjs:264-281`, `verify-referential.mjs:71-132`) | design-decision ⧗ | Cooperative single-operator tool by charter. Runtime requirement: host-issued identity tuple (ADR-007). |
...
| A2-005 | low | bodyless-grace reclaim window (`append-lock.mjs:159-203`) | design-decision | Documented trade-off in code; not to be re-fixed blindly. |
| A3-001 | critical | `POST /bridge/approvals/respond` appends `APPROVAL_DECIDED` for any `approvalId` with no open/already-decided check (`bridge-routes-approvals.mjs:61-79`); the CLI path refuses (`approval.mjs:57-78`); projection keeps every row (`projections.mjs:288-300`) | confirmed ⧗ | Coordinator read `server.js`: loopback-only, writes require the bridge token — so a same-machine token holder, not an anonymous caller. Still a real CLI/bridge inconsistency. **Fix candidate (separate PR):** mirror the CLI check atomically before the append. Runtime: single-use bound handles (ADR-005). |
| A3-002 | critical | a required gate with no file yields no run; `_gates-before-done.mjs:69-70` filters present runs only → `goal done`/`plan complete` proceed under strict | confirmed ⧗ | Coordinator read `commands/ci.mjs:204-222`: `maddu ci` **already fails closed** ("required but no runnable gate resolves"). The gap is only in the completion check. **Fix candidate (separate PR):** port `ci.mjs`'s required-integrity check into `_gates-before-done.mjs`. |
| A3-003 | high | an operator file under `.maddu/gates/` may shadow any builtin gate id; "later-loaded wins" (`gates.mjs:78-90`) | design-decision ⧗ | Documented operator override for the repo-local tool. Runtime requirement: trusted registry with implementation digests (RFC §7.3). |
| A3-004 | high | an import-time throw is keyed by file path, so a required id never matches it (`gates.mjs:34-44`) | confirmed ⧗ | Same blind spot as A3-002 for the completion path; `maddu ci` catches it via count = 0. Fix together with A3-002. |
...
| A4-001 | high | redaction is secret-shaped only; a foreign path survives event data, error text, and OTLP export (`secret-scan.mjs:21-84,217-271`, `otel.mjs:74-134`, `paths.mjs:69-75`; probe-confirmed) | design-decision ⧗ | `hard-rules.md` rule 6 already says best-effort pattern redaction is not a privacy boundary. Runtime requirement: references-only evidence, keyed commitments (ADR-008). |
...
| A5-001 | high | `spine.append()` has no operation-id/idempotency parameter; a crash-then-retry duplicates the logical event under two ids (`spine.mjs:620,774-775,876-891`) | confirmed ⧗ | Runtime requirement: unique operation ids + reconciliation (RFC §8, ADR-003). Not a dev-tool defect on its own. |
...
| A6-001 | high | autonomy scoring is keyed by the bare `lane` string joined from self-reported registration/claim events; a new lane name starts at n=0 (`autonomy.mjs:105-130,183-217`) | design-decision ⧗ | Lanes are ad-hoc by design (`docs/lanes.md`). Economy requirement: host-issued principal lineage, never lane ids (ADR-007). |
...
| A7-001 | high | no test forces the `GATE_RAN` append to throw, so the empty catch at `gates.mjs:194` has zero coverage | confirmed ⧗ | Runtime requirement: explicit evidence-durability result (RFC §7.3, P3). A characterization test that pins the current swallow is a P3 precondition. |
...
| A8-001 | high | **process finding, not code:** auditors read the live checkout while the coordinator committed the RFC (`25f9cea`) and P1 (`61ca780`) on it; `git cat-file -e 227c69d:docs/57-…` fails though the file is on disk | confirmed (orchestration) | Mitigated in this round: A4, A5, A8 and the cartographer each ran `git diff --stat 227c69d..HEAD` (or read via `git show 227c69d:<path>`) and found every runtime/gate/test file byte-identical; only `docs/57`, `docs/00-index`, `docs/rfc/*` and the two new tests differ. **Next round:** give every read-only auditor a detached worktree at the pinned SHA (`git worktree add --detach`), never the coordinator's working checkout. |
...
PENDING — the verify stage is running. Each critical/high row above receives
its refuters' labels and reasoning here; any row a refuter marks
`not-supported` is removed from the register with the reason kept.
...
PENDING — from the completeness critic; the coordinator's own list:
authorization (A3), tenant isolation (A4), signing trust (A2-003),
durability acknowledgement (A2-004/A5-003), settlement atomicity (future
P6, informed by A5-001), privacy retention (A4-001), incompatible contract
change (A7-002, now pinned).
...
PENDING — the critic's list of additional ADRs (if any) and the coordinator's
