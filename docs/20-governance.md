...
| event-schema-complete | safety | every `EVENT_TYPES` key carries an explicit, well-formed entry in the published event contract (`event-schema.mjs`) — the contract cannot silently lag the emitters | v1.94.0 |
...
| replica-untracked | fail | a team-sync checkout never git-tracks `.maddu/config/replica.json` — a committed replica id duplicates across clones and forks the partition chain | v1.96.0 |
