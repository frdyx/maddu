...
**First tier of the 2026-07-16 fleet usage-audit roadmap** (22-workspace spine audit + Codex analysis, `docs/audit/fleet-usage-audit-2026-07-16.md`): before changing what the instruments measure, make the instruments honest. Verbs **72/72**; gates **74**; event contract **1.7.0** unchanged (no new types — one disposition reclassified). All checks are auto-discovered self-tests.

- **Version SSOT** (`installed-version.mjs`): one resolver — `maddu/version.json` → `version.json` → `maddu.json.framework_version` → honest `unknown` — now shared by **both** `maddu fleet` and `maddu insights`, each printing the source it resolved from. The audit had caught them disagreeing on the same repo (fleet 1.15.0 vs insights 0.19.0) because insights replayed the spine's install-time `FRAMEWORK_INSTALLED` version, which upgrades never touch. Both surfaces now agree 22/22.
- **Import/native segmentation, keyed on `data.source`** — not on event type, because `TOKEN_USAGE_REPORTED` is *also* emitted natively by worker wrappers. Imported backfill rows (`source: 'claude-code-transcript'`) no longer move activity/recency/volume: snyggare drops from a misleading 55,225 to its native 1,557 events (+53,668 imported, reported apart). `--include-imported` restores merged totals. A backfill-only type classifies **imported-only**, never `dead`. Both directions are tested (imports excluded AND native rows of the same type kept).
