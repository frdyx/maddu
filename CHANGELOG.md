...
- **The law.** A token-ledger row is priced only when PROVABLE; **estimated vs wire-reported dollars are never merged**; null ≠ zero ≠ omitted survives end-to-end (`Object.hasOwn` threading through the tokenLedger reducer — a present-but-mistyped value drops toward the unpriced bucket, never coerces). **Estimation is READ-time only**: an estimate is presentation over the factual record and is never written to the append-only spine (it would bake a stale price into history). The spine records facts: `pricingIdentity {authority, model}` when provable, and contract-ready `costUsd` + `costProvenance:'wire-reported'` (no emitter reports wire cost today).
- **The manifest.** New stdlib-only `lib/pricing.mjs`: embedded versioned `PRICING_MANIFEST`, exact byte-wise `(authority, model)` match or nothing (lowercase-hostname authority grammar, `YYYY-MM-DD.N` version grammar, unique pairs, four finite rates ≥ 0, unknown keys rejected). `.maddu/config/pricing.json` override read-if-present — entries REPLACE by key; a parse-error or schema-invalid override is a **loud exit 2 before every branch** (`--unreported-count` included), never a silent fallback; effective version surfaces as `<embedded>+override`.
- **Provable identity.** Runtime descriptors gain optional `authority`, grammar-checked at the spawn seam (descriptors load unvalidated) — valid injects `MADDU_PRICING_AUTHORITY`, invalid omits with a one-line diagnostic, and the seam **unconditionally scrubs the inherited env first** (funnel r1: a nested worker routed through a different endpoint would otherwise inherit the parent's authority and silently misprice). Wrappers stamp `pricingIdentity` only from `pricingModel` — the model **parsed from the provider's own stream**, passed separately from the display model (funnel r1: a `-unknown` fallback or gemini's unproven `MADDU_MODEL_HINT` never becomes factual provenance). `usage import` always omits — a transcript cannot distinguish api.anthropic.com from Bedrock/Vertex.
- **A dollar figure is exact or absent, never a rounded lie.** Truth table per row: reported (`costUsd` number + `wire-reported`) / estimated (identity + exact match + both token counts; null/absent cache components excluded — never zeroed — and flagged `partialComponents`) / unpriced with reason (`no-pricing-identity` | `no-manifest-match` | `unreported-tokens`). IEEE extremes fail loudly in BOTH directions (funnel r3+r4): divide-before-multiply keeps representable products, a non-finite estimate or bucket sum throws (never `JSON.stringify(Infinity) → null`, a fabricated empty bucket), and a positive term underflowing to exact 0 throws (never a fabricated proven zero). Text display floors sub-0.0001 nonzero at `<0.0001` (funnel r2); `--json` keeps exact numbers.
- **The surface.** `cost --usd` text: usdRep/usdEst/unpriced columns (blank = no bucket members; `0.00` = proven zero) + manifest-version and unpriced-reason trailers (empty-ledger path included). `--json --usd` is GROUP-shaped: per group `usd {reported|null, estimated|null, unpricedCount, unpricedReasons, partialComponentsCount}` + top-level `pricing {manifestVersion, overrideActive}`. Without `--usd`, byte-compatible with pre-S4. Cockpit `renderCostCard` + `/bridge/cost` deliberately untouched (goldens byte-identical); the `token-ledger-schema` gate deliberately not extended (a new required field would hard-fail every pre-existing row). Known out-of-scope law violation left cited: `cost-budget.mjs`'s `Number()||0`.
- **Contract 1.15.0 → 1.16.0** — `TOKEN_USAGE_REPORTED` gains `costUsd` (`number|null?`), `costProvenance` (`string?`), `pricingIdentity` (`object?`) — honest-optional, additive; baseline refreshed with this release. Expect most existing rows **unpriced** — that is the mechanism being honest, not a defect.
- **Suites:** pricing (43) · cost-provenance (38, spawned-CLI end-to-end incl. poisoned-parent spawn-seam scrub, mixed-provenance group, proven-zero vs empty bucket, reducer type-drop, override precedence + loud failure, over/underflow, wrapper emission gate).

## [v1.117.0] · 2026-08-06 · buzz-steals S2: workspace identity in the chain preimage

**Workspace identity in the chain preimage** (plan `pln_20260805152106_44d2` phase `s2-workspace-identity`, the block/buzz audit's chain-of-integrity steal; plan **Codex-SOUND after 6 review rounds**; diff **Codex-CLEAN after 24 review rounds**, findings **5 → 5 → 7 → 3 → 2 → 2 → 2 → 2 → 1 → 1 → 1 → 1 → 1 → 1 → 1 → 2 → 5 → 1 → 1 → 2 → 1 → 1 → 1 → CLEAN** — 49 fixed: 23 blocker, 22 major, 4 minor). Self-test **200/201** stable across 12+ full sweeps (the one red is the pre-existing local atlas architecture-drift — environmental, CI clean); ~310 assertions across 3 new + 3 recalibrated suites; contract **1.14.0 → 1.15.0**.

- **The gap.** `prev_hash` proved recorded history wasn't altered *internally* — nothing bound a line to THIS workspace. A chain-valid line lifted from another workspace's spine (or an entire internally-consistent foreign partition) verified green.
- **The identity.** DERIVED, never minted: flat mode = `ws_` + 16 hex of the committed genesis line's sha256 (exact `hashLine` semantics — retroactively identical for every existing workspace, zero ceremony); sync mode = frozen by the in-band `WS_IDENTITY_ANCHORED` event (merge-first nomination pinned by immutable position + hash, published exactly once through ONE serialized law with a history-compatibility refusal — an anchor that would invalidate already-stamped events is never published or adopted). Genesis/bootstrap/anchor/resolution lines are ws-less by protocol (no self-reference); `ws` rides INSIDE the stored line and therefore inside `prev_hash` — a splice cannot re-chain without wholesale recompute, which the flat-mode OTS anchor then pins in time.
- **The conflict ceremony.** Racing offline first-writers freeze S2 writes (`WS_IDENTITY_CONFLICT`); `maddu spine identity resolve --keep <ws_…>` appends `WS_IDENTITY_RESOLVED` — validated ATOMICALLY under the funnel lock against a fresh anchor scan with an exact canonical `{eventId, genesisHash, spineIdentity}` tuple binding, and carrying a forward **cutover** (per-partition chain heads): verify grandfathers LOSING-identity stamps at-or-before their bound head — the only forward-only recovery for both-sides-stamped conflicts — and **same-selection extensions** with fresh heads heal pre-adoption offline work (auto-appended by `spine sync` post-pull when needed).
- **Fail-closed everywhere.** One strict enumeration law across every identity sweep (only ENOENT is absence; invalidly-named segment-bearing partition dirs refuse); PARSE is authoritative (substring prefilters are JSON-escape-evadable); the **committed-element law** end-to-end (an unterminated final line is not part of the record — for writers (TORN_TAIL refusal), readers, strict readers (counted as an accounting gap), verify (torn in EVERY segment, outside the chain), nominations, cutover heads, residual migration, and sync import (a buried torn element is fatal)); the in-lock final gate revalidates EVERY write (stale stamps refuse and restamp bounded; missing ws late-stamps when an authority exists).
- **Team-sync fencing.** `spine sync`'s snapshot commit + upstream probe + pull run in ONE uninterrupted hold of the active partition's append lock (peer AUTHORITY bytes can never interleave with a local append); sync-init runs marker→migration→activation under the flat funnel with a hard anchor-bootstrap precondition and named resumable failures; an **unattached clone** (partitions, no replica.json) refuses every write centrally (`REPLICA_UNATTACHED`) with a pinned S1-drain recovery exception for exactly `spine sync init`.
- **Downstream honesty.** The wrapper stamps only provably-fresh cached identities (committed line-aligned `{raw, committed}` fingerprints with stat-reuse; drops in sync mode when unprovable); verify/oversight/decisions carry the new FAIL kinds (`ws_mismatch`, `ws_anchor_conflict`, `ws_identity_unverifiable`, WARN `ws_cache_stale`); the fresh `spine sync init` path declares its census-pinned `witnessRaw('sync-init-activation')` (an uncalibrated S1 false breach found by this funnel); `append-lock` gained a bounded Windows EPERM retry.
- **Contract 1.14.0 → 1.15.0** — envelope `ws` (`string?`, honest-optional), `WS_IDENTITY_ANCHORED`, `WS_IDENTITY_RESOLVED` (with `cutover` `array?`); baseline refreshed with this release (fingerprint `43bdc673`). Forward-only: ws-less legacy events verify exactly as before.
- **Suites:** ws-identity (96) · ws-sync-identity (51) · ws-chain-splice (21) · recalibrated: spine-sync-init (94) · spine-partition (21) · token-wrapper-emission (29).

## [v1.116.0] · 2026-08-05 · buzz-steals S1: mutation-witness coverage guard

**The mutation-witness coverage guard** (plan `pln_20260805152106_44d2` phase `s1-mutation-witness`, born from the block/buzz audit's EmitGuard steal; plan **Codex-SOUND after 6 review rounds** — 27 findings, 10 blockers; diff **Codex-CLEAN after 6 review rounds**, findings **8 → 3 → 1 → 9 → 2 → CLEAN** — 23 fixed: 17 blocker, 5 major, 1 minor). Self-test **197/198** (the one red is the pre-existing local atlas architecture-drift — environmental, CI clean); ~98 new assertions across 5 suites; contract **1.13.0 → 1.14.0**.

- **The gap.** The spine's `prev_hash` chain proves recorded history wasn't altered; nothing proved a mutating seam RECORDED AT ALL. A verb or bridge route that mutated and exited 0 silently was invisible — the runtime complement to the static tier census was missing.
- **The verdict law.** A mutating seam that exits 0 with zero spine appends and no declared no-op breaches: one `MUTATION_UNWITNESSED` event (new type, dormant disposition — `DISCIPLINE_SKIPPED`'s sibling), a one-line stderr signal, exit forced to 1. Non-zero exits never breach (a crash is not silent); read shapes never breach. Silence is the only red; excuses must be declared — ~40 append-free success paths now carry honest reasons (idempotent / machine-scope / host-file / host-process / derived-state / config-write / conditional-append classes), and the funnel de-excused two shapes that actually append (`mailbox read`, `runtime detect`).
- **Two-stage CLI emission.** Commands call `process.exit()` directly (491 sites), so the verdict runs in a `process.on('exit')` handler: breaches spool one O_EXCL file each to `.maddu/state/mutation-breaches/`, and the NEXT dispatcher run drains them onto the spine BEFORE arming the command witness (a drain append can never credit the upcoming command). Per-file atomic rename claims carry `claimedAt-host-pid-nonce` (mtime is never the staleness signal); same-host reclaim only on proven-dead PID; exactly-once normal path with `hasBreachId` spine-scan + `.drained` consume-markers closing the crash window; unparseable rows quarantine to `.corrupt` and keep redding the census until disposed.
- **The bridge.** Per-request AsyncLocalStorage context; breach only after a 2xx response (4xx/5xx are already loud); a pinned `READONLY_POST_PATHS` table (4 routes, censused); plugin handlers declare via `{handled, witness:{kind:'noop', reason}}` — a silent plugin write breaches inline; if the inline append itself fails the breach falls back to the CLI spool (the response is never rewritten). Witnessing parity fix: `/bridge/trust/audit` now appends `TRUST_AUDIT_RAN` like the CLI (only on a real audit, with the real `depsFingerprint`; refusals declare instead of minting a fake clean audit).
- **readShapes.** `_tiers.mjs` entries gain declarative read shapes (string tokens or `{tokens, requiredFlags}`, mutation-wins precedence) so `plan list`, bare `memory`, `spine anchor --verify`, `upgrade --dry-run` and ~45 more read-shaped invocations of mutating verbs never breach — every claim verified by the census (stale claim = FAIL, arm-anchored mutation-signal scan = WARN).
- **The census.** `command-tier-discipline` extended (gates cap stays 75): dispatcher arity (one guarded `runWithWitness` site + two pinned read-only `--help` arms), readShapes honesty, pinned direct-writer exceptions (init genesis ×3, fleet delegation), bridge wiring + exemption-table existence, and the runtime backstop (undrained rows / stale claims / quarantine each red separately).
- **Hard-won invariants.** Witness state anchors on `globalThis` (module-identity across cwd-first/template-tree imports stranded credits — found live, twice); bin resolves ONE lib dir and goes inert when the witness lib is missing there (a cross-dir fallback would false-breach every verb on an old install); zero-credit regressions pin that the session-start fire, `mailbox read`, and `goal set` happy paths can never hide behind an excuse.
- **Contract 1.13.0 → 1.14.0** — `MUTATION_UNWITNESSED` added; baseline refreshed with this release (fingerprint `4a520601`).
- **Suites:** mutation-witness (16) · mutation-breach-drain (21) · mutation-witness-cli (18) · bridge-mutation-witness (25) · mutation-witness-census (18).

## [v1.115.0] · 2026-08-05 · memory-recall track: trust-gated recall, BM25 search, wiki heal, MCP facade

**The memory-recall track** (plan `pln_20260803035706_7040`, born from the TencentDB-Agent-Memory comparative brief; diff **Codex-CLEAN after 10 review rounds**, findings **14 → 6 → 9 → 9 → 5 → 5 → 2 → 3 → 3 → CLEAN** — 36 fixed: 7 blocker, 20 major, 9 minor). Self-test **189/189** (4 optional-dep skips); ~250 new assertions across 9 suites; contract **1.11.0 → 1.13.0**. Landing acceptance ran against the live spine: wiki sync appended **462** missing blocks (726 total, drift → 0), and a real `brief --for-agent` fed 3 operator-approved facts with hash-bound witnesses, gate green.

- **The defects.** CLI slice-stops ran hindsight but never the wiki (only the bridge route did both) — 462 of 726 live stops missing from the wiki at fix time, and `appendSliceStop` had no idempotency. Cross-corpus search was substring + timestamp ordering: recency always beat relevance. Extracted facts had no trust model — 726 of 935 current facts are auto-promoted regex summaries, so feeding memory back to agents would have amplified unreviewed agent assertions.
- **One slice-stop path.** `materializeSliceStop` (new `slice-materialize.mjs`, session-lifecycle precedent) is called by BOTH transports: hindsight extraction + idempotent wiki append (page-scan on the `- **Event:** <id>` marker — replays are byte-identical no-ops). New **`maddu wiki`** verb: `sync` (append-missing-only backfill, preserves hand-edits), `drift` (real membership counts, sanitizer-collision aware), `page`, `list`, `rebuild --force`. Also fixed the stale `.maddu/state/memory.ndjson` path print and the wrong-directory mkdir in `ensureMemoryFile`.
- **BM25 relevance.** New pure `relevance.mjs` (k1 1.2 / b 0.75, dotted-token tokenizer, tag/lane boosts) — corpus stats computed over the FULL per-kind corpus before filtering so IDF stays honest; rows deriving from one source event collapse to the best representative (+`also`, capped, with honest totals). Golden suite Recall@5 **1.000**, MRR **0.96**, p95 **6ms**; `--sort time` preserves legacy ordering. Memory rows carry hash-validated trust labels everywhere.
- **Event-sourced fact trust.** Every fact starts `asserted` — searchable, never injectable. `maddu memory approve` binds an approval to a sha256 of the WHOLE canonical fact (strict-keyed, type-validated, injective — any field edit voids it, malformed shapes are unapprovable); `revoke` requires a reason. Trust survives `rebuildMemory` (spine wins), and event-recorded supersessions retire facts even when the fact file missed the replacement.
- **The recall packet.** `buildRecallPacket` (new `recall.mjs`) is the single eligibility seam: approved-only, ranked, hard-capped at 8 items / 16KB measured over the canonical content (ids, tags, lane included), with complete per-reason withheld accounting (rows capped, counts never lost). `brief --for-agent` re-derives trust immediately before witnessing, records `MEMORY_INJECTED` with per-fact `{id, sha256, approvalEvent}` rows, witnesses refusals, and FAILS CLOSED on any error — fact text renders witnessed or not at all (including `--dry-run`: nothing renders). `maddu memory recall [--json]` inspects without injecting; `GET /bridge/recall` mirrors it read-only.
- **The critical gate.** `memory-injection-bounded` (gates 74 → 75, attributed) is a pure chronological spine fold over UNCAPPED history: bounds + per-witness approval-reference validation (exists, binds this fact to this hash, not revoked between) hard-fail; temporal anomalies a team-sync-merged order cannot prove causal (approve-after-feed, revoked-between, superseded-between) surface as named WARNs — detected, never silent, never a false critical across legitimate revoke/reapprove epochs.
- **Máddu as an MCP server.** `maddu mcp serve` (verbs 72 → 73, attributed) — hand-rolled newline-delimited JSON-RPC 2.0 over stdio, Node stdlib only: `memory_search`, `recall_packet` (inspection ≠ injection: no witness), `wiki_read`, `atlas_query` (answers `atlas_unavailable` until the atlas track lands — intended), `status`. Every response size-shaped; strict string/number ids; the conformance fixture hash-proves spine+memory+wiki untouched by a full dialogue.
- **Hardening (the funnel's yield).** Whole-fact canonical hashing ended field-enumeration whack-a-mole; witnesses reference approval EVENTS so validation survives cross-partition reordering; strict fact well-formedness blocks unknown-property smuggling while keeping every real writer (learn/vendor/evolve) approvable; one corrupt row can no longer crash search, recall, brief, or any MCP tool; caller-controlled inputs are clamped before use AND before echo on every new surface.
- **Contract 1.11.0 → 1.13.0** (1.12.0: MEMORY_FACT_APPROVED/_REVOKED; 1.13.0: MEMORY_INJECTED, MEMORY_INJECTION_REFUSED; all additive) plumbed through EVENT_TYPES, schema, dispositions, both doc trees + baseline.
- **Suites:** relevance-bm25 **24**, search-relevance-golden **17**, memory-trust **34**, recall-packet **39**, memory-injection-bounded **16**, mcp-serve **17**, wiki-materialize **14**, wiki-cli **19**, plus golden fixture `search-golden-queries.json`.

## [v1.114.0] · 2026-07-24 · worktree two-resource recovery (detach-side)

**PR-D of the session-lifecycle track** (plan v7, Codex-SOUND; diff **Codex-CLEAN after 6 review rounds**, findings **13 → 8 → 8 → 5 → 5 → 2 → CLEAN**). Drains the deep worktree residual deferred from PR-C. Self-test **185/185**; new suites worktree-lock **26/26**, worktree-recovery **42/42**, worktree-recovery-fold **17/17**, worktree-writer-census **9/9**; contract **1.10.0 → 1.11.0**.

- **The defect.** `detachLaneWorktree` removed the git checkout **before** appending the terminal `WORKTREE_DETACHED`, so a crash between stranded the lane forever: `readAttachments` folds the attachment **LIVE** (a pure spine fold, no fs check), plain release is blocked, a retry throws on the null-`branchHead` ancestry probe, re-attach phantom-reuses, and nothing self-heals. Hard rule #2 forbids a rollback-atomic write — the two resources (the **attachment** spine record + the checkout **directory**) could drift apart with no recovery.
- **Intent-first detach.** A removing disposition (`merged`|`abandoned`) now appends a durable **`WORKTREE_DETACHING`** intent **after** dirty/ancestry authorization and **before** git removal, then removes, then postconditions the leaf (`lstat` ENOENT), then appends the terminal. A crash between is recoverable from the intent — never by inferring "gone" from filesystem absence (a storage outage is indistinguishable). `kept` stays a direct single-append terminal (no intent, no removal). Every git probe's exit code is checked (a failed `status`/`rev-parse`/`merge-base` **refuses**, never reads "clean"). A partial (intent landed, removal/terminal not) reports the committed spine **envelope** ids.
- **Physical identity.** Every attach mints a per-worktree **`worktreeInstanceId`** token into the checkout's **private** git dir (`git rev-parse --absolute-git-dir` — never the shared `info/`), load-bearing (atomic write + read-back, else the provisioning rolls back and no `WORKTREE_ATTACHED` lands). Recovery removes **only** a token-matched **present** instance, so a manually-replaced checkout is never deleted. Legacy (pre-token) attachments detach **directly** (honest git-probe-authorized removal — no minted-token pretense) and stay ineligible for auto-finalize.
- **Positive-removal-only auto-finalize.** A **present-instance** pending intent **self-heals** inside the janitor sweep (`recoverPendingDetaches`, run once inside `runJanitor`): full rule-#9 gauntlet (`janitor:worktrees` allowlist + finite cooldown, both rechecked **inside** an atomic-publish **global recovery lock**), a token-aware pre-scan, and `TRIGGER_FIRED` only when ≥1 present token-matched candidate is lock-held (so a pure no-op round burns no cooldown). An **absent** instance is **never** auto-terminalized — it is surfaced `needsOperator`; `/projection` reprojects on a finalize and prints the exact `--recover` command per case.
- **Operator `--recover`** (`maddu lane release <lane> --worktree --recover`) — active-owner-aware authorization (an inactive/closed owner lets any active operator recover) × a physical-state × origin matrix: present+token-match → remove + terminalize; present+mismatch → orphaned-leftover (**never** removed); absent+intent → preserve the verified disposition; absent+intent-less → orphaned compensation; **foreign origin → refuse + redirect** to its source replica. Records both `recoveryActor` and `attachmentOwner` (honest provenance, never impersonating the closed owner).
- **Team-sync safe.** A **device-local** replica lineage (`$GIT_DIR/maddu-replica-lineage.json {current,predecessors,complete}`, never synced) classifies each intent **and** attachment origin (local / foreign / unverifiable) from one **strict** partition snapshot (fold + provenance share it — no TOCTOU). A foreign attachment is never terminalized locally (that terminal, synced back, would close the source replica's real attachment). Auto-finalize requires **every** stream — local and foreign — to strict-account; a torn or unreadable partition fails **closed**.
- **New atomic-publish lock** (`worktree-lock.mjs`) serializes every worktree writer (attach / detach / finalize / operator-recover / force-preflight / inline-release) in **disjoint** namespaces (`worktree-lanes/<lane>.lock` vs `worktree-recovery/global.lock` — `recover` is itself a valid lane slug). A published lock always carries its owner record (proven-dead reclaim always works), dead owners are stolen by an atomic `.dead.<ownerId>` quarantine (retained), release renames aside then deletes (never in-place), and the finite wait degrades to a structured `lock-busy` skip.
- **Contract 1.10.0 → 1.11.0** (`WORKTREE_DETACHING`; `worktreeInstanceId?` on ATTACHED/DETACHED; `recoveryActor?`/`attachmentOwner?` on DETACHED) plumbed through `spine.mjs` EVENT_TYPES, dispositions (dormant), `verify` lifecycle rules, both doc trees + baseline. A **production-only worktree-writer census** keeps `worktrees.mjs` the sole appender of all three events.
- **Backfilled:** the missing **v1.112.0** entry below.

## [v1.113.0] · 2026-07-23 · lane-writer serialization + mode-aware ownership

**PR-C of the session-lifecycle track** (plan v4, Codex-SOUND; diff Codex-CLEAN after **5 review rounds**, findings **8 → 6 → 3 → 1 → CLEAN**). The locking/ownership tier split out of PR-B. Closes the read-decide-write race every lane-ownership writer left open — and the `POST /bridge/lanes/release` **authorization hole**. Self-test **181/181**, concurrency matrix **48/48**, census **10/10**, force-discipline **18/18**; **no new event types, no spine schema change**, `project()` output byte-identical (golden-tested).

- **One serialized appender** (`lane-ownership.mjs`, new): every `LANE_CLAIMED` / `LANE_RELEASED` / `LANE_CLAIM_FORCED` mutation now decides **and** appends inside one transaction, against a fresh strict snapshot taken **under the lane-claims lock** (`lane-claims-lock.mjs`, new: `withClaimsLock`), deciding **mode-aware from one ownership fold** (`ownersOf`). A lane-writer **census tripwire** fails if any raw ownership append reappears outside the helper (it follows namespaced + aliased imports). `spine.append` serializes only the physical write + hash chain — never the decision window above it, which is exactly the race this closes.
- **Lock model:** active-validating writers (claim / force / auto-claim) take `close → lane-claims`; release + the janitor orphan pass take lane-claims only. Global order `binding → close → lane-claims → active-pointer` (SSOT'd in the session-lifecycle header). `withCloseLock` gains a per-call `maxWaitMs` so auto-claim skips a busy lock (~0 wait) instead of stalling the editor.
- **Release is now authorization-gated (the headline fix).** `POST /bridge/lanes/release` previously appended `LANE_RELEASED` for **any** `sessionId` with no holder check — in default mode a release clears the lane, so **any caller could evict any lane's real holder**. Release now requires the actor to OWN the lane (holder or, under team-sync, a superseded owner): **404** no-claim / **409** owned-by-another, holder intact. Worktree disposition runs in-lock (owner re-read → git op → release append) and authorizes by the **attachment owner** (`liveAttach.session`), not the claim holder, so a superseded owner can neither orphan its own live checkout nor be blocked from withdrawing.
- **Claim** requires an active **registered** session and refuses on **any** active rival owner (not just the current holder); inactive orphans are cleaned up first (both modes). **Force** is the sole evictor of an active rival — it releases every current owner and stamps an import-stable `data.forceGroup` id on every preempt-release + marker + trailing claim; a `--force` over a free/self-only lane degrades to a plain claim. Multi-append writers honor a partial-failure contract (fixed order, first-failure stop, success only when the terminal claim landed, committed-event ids + decision-time holder surfaced; recovery re-snapshots — never replays a stale release).
- **Force-discipline gate:** reconstructs each bundle by `forceGroup` id (import-stable, not spine contiguity) and, in **default mode**, reconstructs the pre-force holder from the pre-marker prefix minus the bundle's own preempt-releases — hard-failing a forged prior (verified against unrelated-fg-event, planted same/cross-lane fg-release, and fg-tagged-claim forgeries). In **sync mode** the reconstruction is **withheld as unsound** (a merged replica history can resurrect an earlier first-claimer by filtering a planted release); sync relies only on the import-stable prior-once-claimed check — forgery detection belongs to the integrity layer, not a discipline gate.
- **⚠️ Deliberate contract changes (migration):** bridge/CLI `lane claim` now refuses an **unregistered or closed** session (previously tolerated → manufactured an immediately-orphaned claim) — register first; `/bridge/lanes/release` is authorization-gated (404/409); the auto-claim fallback lane is now `auto/<full-session-id>` (was a last-6 suffix) so two sessions sharing a 6-char tail can't collide onto one lane.
- **Scope / residual:** deep worktree two-resource recovery (a `WORKTREE_DETACHED` append-failure that leaves a removed checkout folding as LIVE) is a documented residual **deferred to PR-D**.
- **Suites:** `lane-ownership-concurrency.mjs` (48 asserts — serialization, bridge-release auth default+sync, superseded-owner cases, no-manufactured-losing-claim, close/unregistered guards, malformed-spine, auto-claim busy-lock, fallback collision, project/reduceClaims parity, per-boundary failpoints incl. sync multi-owner force with `committed == landed`), `lane-writer-census.mjs` (10), plus `ownersOf` (16) and `lane-force-discipline` grown to 18 (all forge variants).

## [v1.112.0] · 2026-07-22 · session-id validation at the chokepoints

**PR-B of the session-lifecycle track** (plan Codex-SOUND after 6 rounds; diff Codex-CLEAN after 4, findings **3 → 2 → 1 → CLEAN**) — the id-validation campaign the PR-A review surfaced, split out as a seeded follow-up. Closes the "an unvalidated `sessionId` reaches a spine writer or a lane-ownership decision" gap the lifecycle work left open.

- **One id grammar, enforced at the chokepoints.** A single `id-grammar.mjs` defines the canonical session-id shape; every ingress that turns caller input into a `sessionId` (CP1–CP5: the bridge body reader `readBodySessionId`, the CLI `--session`/`MADDU_SESSION_ID` resolution, register, heartbeat/close, and the lane-ownership entry points) validates against it **before** the id can key a decision or land on the spine. A malformed id is refused at the boundary, never interpolated into a projection key or an event.
- **`sid-surface-census`** — a tripwire that enumerates every place a `sessionId` crosses from untrusted input into the trusted core and asserts each one routes through the validator, so a new ingress can't silently reintroduce the gap.
- **Fail-open by `stat` ENOENT** where a missing session file legitimately means "no session", distinct from a malformed id (refused). No contract change beyond the stricter ingress validation. Lessons: never smoke-mutate the live spine (temp repo only); the census is the tripwire that keeps the invariant honest.

## [v1.111.0] · 2026-07-22 · session-lifecycle serialization + honest dirty-age

**PR-A of the session-lifecycle track** (plan v27, Codex-SOUND after 27 review rounds — the id-validation campaign the review surfaced is split out as a seeded follow-up, PR-B). Fixes the two operator-queue framework issues (active-session slot corruption; dirty-age escalation counting scratch churn) plus **six pre-existing shipped bugs** the review funnel surfaced. Verbs **72/72**, gates **74/74**, **no contract change** (`SESSION_CLOSED.handoff` now actually CONFORMS to its declared `object|null` schema — see below).

- **Session-lifecycle transactions** (`session-lifecycle.mjs`, new): every lifecycle mutation goes through serialized, condition-checked primitives — `closeSessionIfActive` (one conditional close for CLI/bridge/hooks/janitor: already-closed → no duplicate event; never-registered → **no verify-invalid append** [pre-existing bug: `session close --session <missing-id>` appended a close `maddu spine verify` FAILs]; in-lock precondition so the janitor can never close a just-heartbeated session), `markSessionStaleIfStill` (one-shot, parallel-sweep-safe), `renewSessionIfActive` (env-idempotent reuse re-proves liveness + heartbeats atomically), `registerSessionUnique` (ALL four registration producers: strict id grammar `ses_[A-Za-z0-9_]{1,64}` for ids we create, duplicate rejection in ANY status — **closed sessions can no longer be resurrected by a same-id re-register** — bounded regenerate for generated ids). Global lock order binding → close → pointer-leaf; two-layer lock shape (`…In` unlocked inner variants); every lock mkdirs its parent and fails toward inaction.
- **SessionEnd closes ITS OWN session** — resolved via the SessionStart binding under the binding lock with a 10 s rebind-freshness guard (the payload cannot distinguish same-Claude-id generations; the rule fails open toward a janitor-reaped leak), conditional close, unbind only on terminal statuses. An unbound end closes nothing. The auto-close note now actually persists (pre-existing bug: the hook passed `--focus`, which `session close` discarded) and is a schema-conformant object.
- **Active-pointer machinery** (`session-active.mjs`): locked unique-temp writes (lock timeout SKIPS — no unlocked fallback to defeat the CAS), compare-and-clear scoped to the closing session, byte-compared invalid-content cleanup with honest cleared/not-cleared messages, repair-on-reuse for absent/corrupt/verified-stale pointers (never a live one), and a **discriminated verified-read union** (`active | unverified | stale | invalid` over a SANITIZED record — a pointer file containing a literal `stale: true` property can no longer be misclassified and cleared; a spine with malformed lines yields `unverified`, never a confident classification; replica mode keeps main's exact tolerant semantics).
- **Hook hardening:** all four fire events bootstrap inside their own fail-open containment (previously a bootstrap failure could exit nonzero and **block an edit or compaction**); pre-tool-use resolution is centralized and grammar-validated before ANY use (auto-claim, enforcement, witness); the containment test seam is `MADDU_SELF_TEST`-gated.
- **Honest dirty-age** (the calibration fix): `uncommitted.ignore` scratch globs in `.maddu/config/discipline.json`; **per-file first-seen clocks** (age = oldest still-dirty file; committing the oldest drops the clock; stored as proto-safe pairs with rollback reconciliation in a versioned `discipline/v2/` counter store + legacy-interlude detection); staged-rename clock transfer (R-only, never copies); **baseline retirement** (pre-existing bug: a path dirty at session start was excluded from counting FOREVER, even after going clean and being re-dirtied); worktree-aware work-root observation with domain-change re-baselining; failed observations and invalid configs suppress the commit gate (allow) and report `dirty=unknown` in the doctor gate instead of a false clean; baseline capture is create-only behind a persisted `baselineInit` marker (a lock-failure or witness-latch write can never harden later enforcement); counter RMW is one locked transaction with a transient fail-open fallback.
- **Projection fixes:** a heartbeat from an active session **clears its janitor stale mark** (pre-existing bug: a renewed session stayed visibly stale forever, permanently suppressing re-detection); `git status --porcelain -z` rename records parsed correctly (pre-existing bug: the source path of every rename/copy was mis-read as a bogus dirty path); new pure `reduceSessions`/`reduceClaims(events, {syncMode})` snapshot reducers; the janitor's two passes each take their OWN fresh strict snapshot (a malformed registration line can no longer fabricate a false `LANE_RELEASED`, and reported counts derive only from appends that actually happened).
- **Docs:** docs/44 gains the serialization model, the concurrent-session residual + `MADDU_SESSION_ID` pinning guidance, and the honest-dirty-age section. **Deferred (PR-B, seeded):** boundary id-validation for the remaining reference surfaces (bridge lane claim/release, `--parent` + existence check), the sid-surface-census test, lane-writer serialization for the janitor's pre-existing intra-pass orphan window.
- **Suites:** new `session-lifecycle.mjs` (55 asserts) + discipline grown to 112 (globs, per-file clocks incl. `__proto__`/malformed-map/unobserved-preservation, R-vs-C rename transfer, workRoot domains, baseline init/adoption/retirement); worktree-coherence reworked to real registrations (23).

## [v1.110.0] · 2026-07-21 · `maddu spine anchor --assess` — the anchored assessment + consume ritual

**PR 6a of the verification-witness track** (plan v9, Codex-SOUND after 9 plan rounds / 16 findings; PR 5 = #289). The `anchored` rung gets its producer: an **interactive operator ceremony** that walks the consume-time checks and records `ASSURANCE_ASSESSED` — a **non-authoritative ledger note**, labeled that way by every consumer. Verbs **72/72** (a flag on `spine anchor`); gates **74**; **no contract change** (the event shape + `validateAssuranceEvidence` shipped with contract 1.10.0 in v1.108.0; this is the producer).

- **The tool's checks can only REFUSE — they never grant the level.** The positive evidence for `anchored` is the OPERATOR's own external Bitcoin-backed `ots verify` run: the ceremony prints the exact command, the operator runs it in another terminal and pastes the output (stored verbatim in the note — redacted BEFORE the 8 KiB cap so a token straddling the boundary can't evade the scrubber; 64 KiB read ceiling), and confirms success explicitly (default No). No verifier is ever executed by the tool; no local state ever asserts external assurance.
- **Local refusal gates (`assessBinding`, run TWICE):** payload↔receipt↔SHA binding (canonical payload bytes; the recorded spine position must hash to `receipt_digest` and name the right `VERIFICATION_RAN`), proof present, and the FULL anchor chain verifying — mid-history deletion/renumbering, prev-chain breaks, forged `ANCHOR_*` events. The second pass runs AFTER the final confirm, immediately before the append, and also RELOADS the age policy: anything that moved during the operator's verification window (anchor bytes, spine, `maddu.json`) refuses with nothing recorded. Refused/declined ceremonies append **nothing**.
- **Age policy:** `maddu.json → witness.maxAnchorAge` (`"<n>d"`, optional, validated hard — invalid refuses fail-closed and `--status` WARNs). The operator enters the attestation date from THEIR verifier output (strict UTC `YYYY-MM-DD`; blank/garbage/FUTURE/stale-beyond-policy all refuse; the tool never parses the paste — parsing it would silently trust it).
- **Containment hardening (in-scope fixes to shipped PR 4 code):** every segment read in spine-anchor.mjs now goes through a symlink-free opener — `.maddu`, `.maddu/events`, and the leaf must be real (lstat; symlinks/reparse points refused, not followed), recorded positions must be canonical basenames with a positive integer line and null replica (`../fake.ndjson` escapes refused as `position-invalid`), and canonical-NAMED non-regular entries FAIL the verify rather than silently skewing chain-head/newest-event scans. Historical `ANCHOR_*` events must carry a **string** payload digest equal to disk on EVERY event (an omitted/falsy field no longer skips the comparison).
- **Non-authoritative everywhere:** `--status` shows "assessed anchored … (non-authoritative)" only while the WHOLE evidence tuple (level + subject + receipt + seq + payload digest + proof digest) still matches the anchor's current state — a superseded assessment (post-upgrade) shows as a WARN, and a `replayed`-level event carrying anchor-shaped evidence never prints as anchored. `--status --json` carries `authoritative: false`; `spine show` prints a stderr disclaimer; `events list`/`tail`, cockpit event rows, and the shared search title all label the event.
- **Honest scope:** `assessed_by` is the literal `operator-ceremony` (the operator's handle rides in the note). Documented residual (threat model §13, SECURITY.md, docs/54): only the NEWEST anchor event's proof digest is disk-checkable — superseded proof-digest claims are unverifiable without retained proof versions (retention was considered and rejected: absence of a retained version cannot distinguish forged from pre-retention). Suffix/all-history anchor deletion still needs a retained operator checkpoint; suppression is silent; an anchor proves a receipt existed by a time — not that the work was good.
- **Docs:** docs/54 consume-ritual section + optional Tier-2 GitHub immutable-releases subsection (operator action, never a framework dependency); threat model gains scenario 13 (the assurance ladder + five residuals); SECURITY.md's witness claim updated (replay + anchor shipped; the "planned, not shipped" placeholder is gone).
- **Suite:** `scripts/test/spine-anchor-assess.mjs` — 75 asserts (73 on Windows + 2 POSIX-gated symlink fixtures): every kill-criterion fixture refuses with nothing appended (forged newest/omitted/non-string digests, mid-history delete/renumber, tampered payload/position, segment escape, symlink leaf + symlinked events dir, stale/blank/garbage/future dates, invalid policy, TTY gate, decline gates, interactive TOCTOU for both proof mutation AND policy tightening at the final-confirm prompt), redact-before-truncate proven with a straddling token, and the 6b boundary asserted (missing replay does NOT block `anchored`).

## [v1.109.0] · 2026-07-21 · `maddu spine verify --replay` — clean-checkout replay of declared commands

**PR 5 of the verification-witness track** (plan v3, Codex-SOUND after 5 plan rounds; PR 1 = #284, PR 2 = #286, PR 3 = #287, PR 4 = #288). The `replayed` rung of the assurance ladder gets its producer: clone this repo at an **exact commit** and re-run the verification the repo itself declares. Verbs **72/72** (a flag on `spine verify`); gates **74**; **no contract change** (`VERIFICATION_RAN` data is forward-compat open — the receipt adds keys under the existing type).

- **Declared commands ONLY, read FROM THE CLONE:** `maddu.json → replay: {install?, verify}` is read at the subject SHA — dirty/newer worktree config can never choose the commands for an older commit, and a commit predating the declaration is honestly `unsupported` (exit 2, **no spine events**, never `replayed`). No lockfile/ecosystem inference exists anywhere in the module; the v1 shape is exactly `{install?, verify}` (unknown keys refused; blank strings refused — a shell no-op must not manufacture a passing receipt; committed **symlink** maddu.json refused — it could resolve outside the clone).
- **Exact-subject discipline:** full commit id in the repo's own object format (40 hex sha1 / 64 hex sha256 — an abbreviation or ref is refused), and the object must **itself be a commit**: an annotated-tag id peels through `^{commit}` to a different object than the receipt would record, so it is refused by `cat-file -t`, never peeled.
- **Strict receipt discipline (audit P3):** setup refusals emit nothing; `VERIFICATION_STARTED {kind:'replay', profile:'replayed'}` is appended only after clone + config validation, and **strictly** — a STARTED append failure refuses pre-run (`spine-unavailable`: replay will not run unrecorded), a RAN append failure exits nonzero and never claims `replayed` (unlike the deliberately best-effort test-recency path). Replay receipts are kind-scoped and never satisfy project/self-test recency.
- **Settlement-proof execution:** each declared command gets a fixed 10-minute timeout (env test seam `MADDU_REPLAY_TIMEOUT_MS`, not config); on expiry the **whole process tree** is killed — Windows `taskkill /pid /T /F` (itself bounded), POSIX process-group SIGKILL — under a universal settlement deadline that starts at kill initiation, so a stalled kill can never hang; a spawn error settles as a fail result, never a rejection or a dangling STARTED. `--json` reserves stdout for exactly one JSON document (child output streams to stderr).
- **Fail-closed cleanup:** the clone is deleted **before** the receipt (with a Windows read-only-pack chmod retry), so `clone_deleted` is a fact; a cleanup failure forces `result:'fail', complete:false` + nonzero exit even when the commands passed (`verify_exit` stays visible) — an incompletely executed replay protocol never reads as a successful `replayed` run.
- **Honest scope, printed every run:** `--no-local` isolates git object copying only — host env, credentials, caches, services, and absolute-path writes are NOT isolated; replay is clean-checkout reproducibility + dirty-worktree contamination detection (the suite proves an uncommitted sabotage edit does not reach the clone). Docs: [55-verify-replay.md](docs/55-verify-replay.md). Suite: verify-replay **41/41** (incl. the Windows grandchild tree-kill and a no-temp-leak sweep).

## [v1.108.0] · 2026-07-21 · `maddu spine anchor` — the external witness (OpenTimestamps)

**PR 4 of the verification-witness track** (plan v3, Codex-SOUND; PR 1 = #284, PR 2 = #286, PR 3 = #287). The first evidence that lives OUTSIDE the agent's authority: a canonical payload committing to a spine receipt is stamped into Bitcoin via the **stock OpenTimestamps client** (a declared ambient tool like git — zero package deps). Verbs **72/72**; gates **74**; contract **1.9.0 → 1.10.0** (`ANCHOR_STAMPED`, `ANCHOR_UPGRADED`, `ASSURANCE_ASSESSED` — the bump covers only the events whose producers ship in PR 4–6a; `ATTESTATION_RECORDED` waits for its own bump after the PR 6b spikes).

- **`maddu spine anchor`** stamps the newest `VERIFICATION_RAN` receipt (or `--event <id>`): payload v1 = `{repo_identity, receipt_digest, subject_sha, event_id, position, chain_head, prev_anchor_sha256, seq}` — commits to the specific receipt AND the broader history; canonical-JSON bytes are the stamped digest. `--upgrade` (oldest-first, event only on real proof change), `--status`, `--verify` (read-only diagnostic, **never assurance evidence**). **No offline queue** — offline says so and exits nonzero with nothing recorded; crash recovery re-stamps the same seq; concurrent invocations serialize under a funnel lock (two-process test proves no fork); **team-sync mode refused fail-closed** (a singular chain head cannot cover a merged multi-replica spine).
- **Adversarial `--verify`:** mid-history renumbering/deletion, edited payloads (caught via the successor's `prev_anchor_sha256` AND the `ANCHOR_STAMPED` event digest), rewritten spine positions, and forged `ANCHOR_*` events are all FAIL-flagged — while the output restates every time that **suffix/all-history deletion is NOT detectable without a retained checkpoint** (operator note of latest seq + digest).
- **Honest assurance framing:** an anchor proves the receipt EXISTED, not that it is true (anchor-a-lie is irreducible; content honesty = replay + ceremony). Bitcoin-backed verification is an **operator action** (`ots verify …`, local Bitcoin Core node or explorer mode with its reduced trust stated) — deliberately not wrapped in a maddu verb, because a runner the agent controls re-verifying its own evidence is actor-as-witness. `validateAssuranceEvidence` ships the per-level REQUIRED evidence shapes for `ASSURANCE_ASSESSED` (producer lands in PR 6a; every consumer labels it non-authoritative).
- **`.maddu/anchors/` is tracked** (durable exception): init's gitignore block gains `!.maddu/anchors/`, the stamp path self-heals the re-include inside Máddu's own block on pre-1.108 repos, and the `maddu-state-untracked` gate allowlists the prefix — anchors travel with the repo; the receipt-bearing spine stays device-local (fresh clones verify continuity + self-consistency; position checks are honestly skipped where the spine is absent).
- **Windows spike (kill criterion):** stock client v0.7.2 stamps/upgrades/verifies on Windows — after the documented shim (python-bitcoinlib needs an OpenSSL DLL findable as `ssl`; Python's own `libcrypto-3.dll` copied as `ssl.dll` works — the `EC_*` symbols live in libcrypto in OpenSSL 3). The refusal hint and docs/54 carry the recipe. `MADDU_OTS_BIN` is a test-only seam (a stub cannot forge trust — events record outcomes). Suite: spine-anchor **36/36**.
...
- **Edit-form only, stated why:** `Write(path)` rules are accepted but never matched by file permission checks in Claude Code v2.1.210+; an explicit `--retire-inert-write-twins` flag retires redundant `Write()` rules (only when the `Edit()` twin exists in the same list) with a printed report — never a silent install side effect. This repo's own settings dropped 6 via the flag.
- **Recorded ownership, preservation proven:** the exact strings an install adds are recorded in `.maddu/state/guardrails.json`; uninstall strips exactly the recorded set (a rule the user hand-authored before install survives; missing record → loud exact-string fallback). Re-install after a `guardrails.ask[]` change retires the stale generated rules. E2e drives the real CLI on a scratch consumer repo: install → **byte-identical** second install → uninstall leaves user content intact (settings re-serialize as 2-space JSON, EOL preserved — content-preserving, format-normalizing, stated in docs).
- **Honest framing shipped in the CLI output and docs:** bypassable harness friction covering the **built-in file tools**; Bash-command coverage is version-dependent and not guaranteed; subprocesses are never covered — they go through the discipline hook's ritual-state gate (which does not know about denied paths), and a process outside the harness leaves no trace. `docs/44-session-hooks.md` gains the full section.
- **Codex round-2 hardening (4 findings):** a lost/absent ownership record no longer lets re-install persist an **empty** record (which turned uninstall into a silent no-op leaving every guardrail behind) — all-rules-already-present warns and leaves **no** record so uninstall keeps its exact-string fallback, partial overlap warns that the unowned matches are treated as user-authored; `clearGuardrailState` only swallows `ENOENT` and warns loudly on any other failure (a stale record can mis-claim later rules); malformed permission shapes (`permissions` as array/scalar, non-array `deny`/`ask`) **refuse the install** (exit 1, file untouched) instead of writing rules that vanish at serialize time or clobber user data; a falsy `guardrails.ask` declaration (`null`/`false`/`0`/`""`) warns instead of silently applying zero rules. **Round-3:** explicit `null` shapes refuse too (silently normalizing them breaks the byte-round-trip), `deny`/`ask` are shape-checked even when a run has no rules to write into them, and an **empty ownership record reads as absent** — install never writes one, clears any found on disk, and uninstall falls back to the exact-string strip instead of a silent no-op. **Round-4:** the ownership record marks which containers install **created**, so uninstall deletes an emptied `deny`/`ask` array (or `permissions` object) only when install created it — a user's pre-existing empty arrays survive byte-identical; a non-object settings **root** (`[]`, scalar) refuses like invalid JSON (properties attached to an array vanish at serialize time — install would have "succeeded" while installing nothing and still recording ownership); an existing-but-unreadable `maddu.json` warns instead of passing for absent (only `ENOENT` means nothing declared). Unit 76/76, e2e 35/35.
...
- **`tracked-source-drift` demoted `critical` → `warn` and removed from the required set.** A cooperative drift signal, not a trust boundary — an actor who can edit the pinned files can re-pin them, so `critical` falsely lent it trust-boundary weight (and the P4 fail-closed rule turns a warn-severity *required* gate RED). `maddu ci pin` only accepts fail-capable gates, so a warn gate cannot be promoted to required — an operator wanting a blocking variant must shadow it with a fail-capable operator gate (local-only, own caveats). Fresh clones now go green with an honest "nothing pinned yet" WARN — the original branch's fresh-CI-red blocker is resolved by honesty, not by tracking the baseline.
- **No reasonless re-pin, no waiver.** `maddu sources rebuild` refuses an absent reason (exit 3) and a flag-shaped or whitespace-only one (exit 2); the `--force` waiver is gone, so `reason` on the record is always a non-empty string. `by` carries the active session id when one exists, null otherwise — attribution is recorded when available, not fabricated.
- **Empty pin config is never green anywhere on the surface.** The gate reports a distinct non-ok "nothing pinned" state, and `sources status` exits 2 on an empty config or zero-match patterns instead of printing success.
...
- **CLI invocation receipts** (`invocation-receipts.mjs` + `bin/maddu.mjs`): every `maddu` entry appends one execution receipt (`ts/verb/sub/exit/ms/sessionId/workspace`) to a **new dedicated corpus** `.maddu/state/invocation-receipts.ndjson` — *not* the spine (telemetry isn't a fact the record must witness) and *not* `operations.ndjson` (a regenerable projection `maddu log` overwrites). Recording is **fail-open** at every step (absent lib, no `.maddu`, unwritable disk — nothing ever blocks or noises the verb; p50 write < 10ms, tested), runs in a process-exit handler (survives `process.exit()`, and captures `init` since state-root resolution happens at exit), and is secret-scrubbed at the write boundary — an unknown command's raw text is never persisted, and `sub` is recorded only for known verbs in token shape.
- **Honest incompleteness by contract:** receipts are an *observed-window* signal — fail-open writes + rotation make gaps structural — so every surface renders the retention window (oldest→newest) and the dropped/unparseable-line count alongside any count, and none may present receipt counts as lifetime totals. Retention is size-capped rotation (5MB/file, one previous generation kept, ≤ ~10MB per repo), declared by the new **`maddu log --window`**. The corpus is device-local: never chained, never synced, never exported, untracked, excluded from spine-integrity verification.
- **`insights verbs` is receipts-first:** per-workspace receipt-backed execution counts (with window + dropped + failure tallies) render first; the transcript scan is demoted to a clearly labeled **legacy mentions signal**. JSON gains a `receipts` section.
- **Gate outcome discrimination:** `GATE_RAN` non-pass states are enumerated in [20-governance.md](docs/20-governance.md) (ok / warn — including explicit soft-warn / fail — including thrown gates; legacy `ok×severity` derivation; pre-schema `other` counted apart, never guessed), `insights` renders a fleet **Gate outcomes** rollup, and the new `gate-fail-path` self-test forces failing fixture gates (critical fail, warn-severity, soft-warn, throwing) and proves the non-pass events **land on the spine**. First fleet run already paid off: the audit's "10,229 pass / 0 fail / 104 other" was the collector misreading the payload — the real fleet held **68 warn + 50 fail**; the fail path was writing all along.

## [v1.100.0] · 2026-07-16 · Usage-audit Tier 1 — instrument truth

**First tier of the 2026-07-16 fleet usage-audit roadmap** (22-workspace spine audit + Codex analysis, `docs/audit/fleet-usage-audit-2026-07-16.md`): before changing what the instruments measure, make the instruments honest. Verbs **72/72**; gates **74**; no new event type — one disposition reclassified, and the event contract takes a deliberate MINOR bump **1.7.0 → 1.8.0** (`INBOX_MESSAGE`/`SKILL_CREATED`/`SKILL_UPDATED` gain optional `source` so every `import submit` path that touches the spine stamps itself). All checks are auto-discovered self-tests.

- **Version SSOT** (`installed-version.mjs`): one resolver — `maddu/version.json` → `maddu.json.framework_version` → root `version.json` *only on the framework source checkout* (a consumer app's own `version.json` can never be misread as Máddu's) → honest `unknown` — now shared by **both** `maddu fleet` and `maddu insights`, each printing the source it resolved from. The audit had caught them disagreeing on the same repo (fleet 1.15.0 vs insights 0.19.0) because insights replayed the spine's install-time `FRAMEWORK_INSTALLED` version, which upgrades never touch. Both surfaces now agree 22/22.
- **Import/native segmentation, keyed on `data.source`** — not on event type, because `TOKEN_USAGE_REPORTED` is *also* emitted natively by worker wrappers. Imported backfill rows (`source: 'claude-code-transcript'` / `'import-submit'`) no longer move activity/recency/volume: snyggare drops from a misleading 55,225 to its native 1,557 events (+53,668 imported, reported apart). `--include-imported` restores merged totals while the imported tallies stay reported apart. A backfill-only type classifies **imported-only**, never `dead`. Both directions are tested (imports excluded AND native rows of the same type kept).
- **Exhaustive taxonomy partition**: every one of the 182 defined event types lands in exactly one of fired / imported-only / dormant-by-design / plugin-owned / dead, and the buckets must sum to 182 — the audit found the old summary leaving 19 plugin-owned types unexplained. The partition equation renders in `insights events` and a self-test asserts completeness + disjointness.
- **Role segmentation**: `maddu insights --role consumer|fixture|self` (self = the framework source checkout, detected by a file test, not a name match) so framework self-development can no longer masquerade as fleet adoption.
- **`SKILL_INJECTION_REFUSED` classification decision** (audit rec #10): the emitter is live (`brief` injection path) but fires only when a matching skill is withheld for untrusted provenance — reclassified **dormant-by-design** as a security refusal witness whose silence is the good outcome. Fleet dead count: **0**.
...
**A 2026-07-09 seven-auditor flaw audit (Codex-verified) found several headline guarantees passing for the wrong reason.** This release closes the externally-triggerable compromise chain, makes the spine genuinely tamper-*detecting* rather than merely warn-y, and lands the P2–P4 hardening (discipline honesty, verification-not-actor-witness, gate scope + CI integrity) — no agent is the sole witness to its own work. Event contract **1.7.0** (P2/P3 bumped it within this release), **182** typed event types; verbs **72/72**; gates **74**. Each tier's plan *and* diff adversarially reviewed by Codex before self-merge.
...
- **P1 — spine tamper-detection, strict + honest** (#268): a *post-cutover* chain (shows a `FRAMEWORK_INSTALLED`/`FRAMEWORK_UPGRADED` ≥ 1.98.0 or a `SPINE_CUTOVER` anchor) FAILs `spine verify` (nonzero exit; reds the `spine-integrity` gate even when capped) on an interior edit, deletion, insertion, reorder, or `prev_hash`-strip. A single funnel-locked flat-append primitive (`appendFlatChained`) is shared by `spine.append()` and the worker token wrapper, so every flat write carries `prev_hash` and concurrent writers can't fork; the wrapper drops on contention rather than blocking. `init` chains its prefix; a `SPINE_CUTOVER` genesis seeds an empty fresh sync partition. Pre-cutover/legacy history stays lenient (`chain_fork`/`chain_gap` WARN) so existing spines never false-FAIL. **Unkeyed**: a determined actor who recomputes the whole forward chain, truncates the tail, or edits only the last event verifies clean (threat-model scenario 11) — the spine's integrity claim is reworded from *evident* to **detecting** throughout.
- **P2 — self-enforcing discipline honesty** (#269): a 5-class Bash classifier (`write | self-disable | remedy | ambiguous | read`) where WRITE dominates self-disable (a compound `uninstall && rm` gates as a write), separators are normalized before boundary-matching, and interpreter `-e`/`-c` payloads are probed for a real write-mode call. Fail-open paths now leave a witness — `DISCIPLINE_SKIPPED`/`ENFORCEMENT_ERROR` at the hook seam (latched per episode; self-disable never latched). The off-switch is guarded: `set-override` is domain-checked + rank-weakening + `--reason`/strict-`--approve`, with a **write-ahead** `GOVERNANCE_OVERRIDE_CHANGED` (aborts on append failure; `--force` waives only the refusals, never the write-ahead). Null-session no longer inherits any-session/claim. Event contract **1.5.0→1.6.0** (+3 types); no new gate/verb.
- **P3 — verification, not actor-witness** (#270): success-eval, project/self/heavy test-recency, and completion-claim now believe a tamper-detecting `VERIFICATION_STARTED`/`VERIFICATION_RAN` spine **receipt** appended from the runner's *in-process* result (read via a parse-clean, uncapped verified read) — never a hand-writable `.maddu/state/*-last-run.json`. `GET` stays non-authoritative (three-state integrity by spine position, no spawn; `allMet`/percent withheld unless integrity-ok + fresh). Completion-claim broadened to confident verification-claims with lane-bound, family-matched proof. Event contract **1.6.0→1.7.0** (+2 types → **182**); no new gate/verb.
- **P4 — gate scope + CI integrity** (#271): `rule-5-no-provider-sdks` scanned a *nonexistent* `<root>/maddu` (0 files → green) → a dual-layout resolver + per-subtree sentinels + `scanned==0` FAIL + a lexically-aware banned-import matcher (16 Codex lexer rounds). SKIP-as-PASS closed (reserved exit **77** for declared-skippable only + `--fail-on-skip` + honest receipt). 19/66 pinned "required" gates were `severity:warn` (could never red) → `ci pin` fail-capable-only + fail-closed required-resolution → repin **66→52**. **Branch protection ON** with a required aggregate **`ci`** check on `main` (merge waits for Actions). Load-bearing bonus: a concurrent `appendFlatChained` during `spine sync init` was *losing* the event (returned `{pending}`; `spine.append` threw instead of honoring the caller-retries contract) → fixed to **retry**, proven by a deterministic seam test.
